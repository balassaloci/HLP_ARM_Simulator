...
let mixedOperandValue op state =
    match op with
    | Register R -> registerValue R state
    | Literal x -> x

let execOperandValue op state =
    match op with
    | MixedOp mop -> mixedOperandValue mop
    | ExprOp (op2, shift, expr) -> leftshift (registerValue op2 state) (mixedOperandValue expr state)
...
let executeArithmetic (operation:ArithmeticOperation)
    ...
    | true ->
        ...
        let op1Val, op2Val = registerValue op1 state, execOperandValue op2 state
